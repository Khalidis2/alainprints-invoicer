...
export default function InvoiceHistory({ invoices, onEdit, onDelete, showToast }) {
          ...
          {invoices.map((inv) => (
            <div key={inv.id} style={s.row}>
              <button style={s.rowMain} onClick={() => setOpen(inv)}>
                ...
                <span style={s.status}>{inv.status || "Unpaid"}</span>
                <span style={s.total}>{AED(inv.total)}</span>
              </button>
              <button style={s.editBtn} onClick={() => onEdit(inv)}>Edit</button>
              <button style={s.deleteBtn} onClick={() => remove(inv)}>Delete</button>
            </div>
          ))}
        </div>
      )}
...
const s = {
  ...
  rowMain: { flex: 1, display: "grid", gridTemplateColumns: "70px 1fr 90px 75px 90px", alignItems: "center", background: "none", border: "none", padding: 0, cursor: "pointer", textAlign: "left", fontSize: 13 },
  no: { color: "#E8792D", fontWeight: 700 },
  name: { fontWeight: 700, color: "#1B2A3D" },
  date: { color: "#8A7F6D", fontSize: 12 },
  status: { fontSize: 11.5, fontWeight: 700, color: "#2E7D8C" },
